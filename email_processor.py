...
if __name__ == "__main__":
    ...
    sys.exit(asyncio.run(main()))
            ...
            return {
                'internal_greeting': 'Hi',
                'external_greeting': 'Dear',
                'formal_closing': 'Regards',
                'informal_closing': 'Thanks'
            }
    
    def classify_email(self, subject: str, sender: str, content: str = "") -> Dict:
        """Classify email and determine required action."""
        ...
        # APPROVAL_REQUIRED patterns
        approval_patterns = [
            'please approve', 'approval required', 'needs approval',
            'for your approval', 'approve invoice', 'authorize'
        ]
        
        # NEEDS_REPLY patterns
        question_patterns = [
            '?', 'can you', 'could you', 'please provide',
            'what', 'when', 'where', 'how', 'why'
        ]
        
        # CREATE_TASK patterns
        task_patterns = [
            'please review', 'action required', 'by friday',
            'deadline', 'due date', 'complete', 'finish'
        ]
        
        # DELEGATE patterns
        delegate_patterns = [
            'forward to', 'loop in', 'coordinate with',
            'technical issue', 'it support', 'server issue'
        ]
        
        # FYI_ONLY patterns
        fyi_patterns = [
            'fyi', 'for your information', 'no action',
            'update', 'newsletter', 'notification'
        ]
        
        # Classification logic
        if any(pattern in text for pattern in approval_patterns):
            classification['category'] = 'APPROVAL_REQUIRED'
            classification['requires_reply'] = True
            classification['priority'] = 'high'
            classification['confidence'] = 0.9
            
        elif any(pattern in text for pattern in question_patterns):
            classification['category'] = 'NEEDS_REPLY'
            classification['requires_reply'] = True
            classification['priority'] = 'high' if 'urgent' in text else 'normal'
            classification['confidence'] = 0.85
            
        elif any(pattern in text for pattern in task_patterns):
            classification['category'] = 'CREATE_TASK'
            classification['create_task'] = True
            classification['priority'] = 'high' if 'asap' in text or 'urgent' in text else 'normal'
            classification['confidence'] = 0.8
            
        elif any(pattern in text for pattern in delegate_patterns):
            classification['category'] = 'DELEGATE'
            classification['delegate_to'] = self.suggest_delegate(text)
            classification['confidence'] = 0.75
            
        elif any(pattern in text for pattern in fyi_patterns):
            classification['category'] = 'FYI_ONLY'
    ...
    def suggest_delegate(self, text: str) -> str:
        """Suggest who to delegate to based on content."""
        delegates = {
            'it': ['server', 'system', 'technical', 'network', 'cpu', 'database'],
            'finance': ['invoice', 'payment', 'budget', 'expense'],
            'security': ['access', 'permission', 'vulnerability', 'audit'],
            'hr': ['leave', 'vacation', 'employee', 'hiring']
        }
        
        for dept, keywords in delegates.items():
            if any(keyword in text.lower() for keyword in keywords):
                return f"{dept}_team"
        
        return "team"
    
    def generate_draft_reply(self, email_data: Dict, classification: Dict) -> str:
        """Generate appropriate draft reply based on classification."""
        
        sender_name = email_data.get('sender_name', 'there')
        subject = email_data.get('subject', '')
        category = classification['category']
        
        # Determine greeting based on sender
        is_internal = '@digitaldubai.ae' in email_data.get('sender_email', '')
        greeting = self.style_profile['internal_greeting'] if is_internal else self.style_profile['external_greeting']
        closing = self.style_profile['informal_closing'] if is_internal else self.style_profile['formal_closing']
        
        # Generate appropriate response based on category
        if category == 'APPROVAL_REQUIRED':
            if 'invoice' in subject.lower():
                draft = f"""{greeting} {sender_name},

Thanks for your email regarding {subject}.

I've reviewed the invoice and it looks good. Approved for processing.

Please proceed with the payment process.
...
I've reviewed your request and approve it. Please proceed.

{closing}"""
        
        elif category == 'NEEDS_REPLY':
            if 'documentation' in subject.lower() or 'api' in subject.lower():
                draft = f"""{greeting} {sender_name},

Thanks for your email regarding {subject}.

I'll compile the relevant documentation and send it to you by EOD tomorrow.

Please let me know if you need any specific sections prioritized.
...
I'll review this and get back to you shortly with the information you need.
...
I've noted this action item and will ensure it's completed as requested.
...
{closing}"""
            ...
            delegate = classification.get('delegate_to', 'the team')
            draft = f"""{greeting} {sender_name},

Thanks for bringing this to my attention.

I'm forwarding this to {delegate} who will be able to assist you better with this matter.
...
Thanks for your email. I've received it and noted the information.

{closing}"""
        
        return draft
    
    def create_task_from_email(self, email_data: Dict) -> Dict:
        ...
        # Create task
        task = {
            'title': f"Action from {sender}: {subject[:50]}",
            'description': content[:200],
            'source_email_id': email_data.get('id'),
            'due_date': deadline,
            'priority': 'high' if 'urgent' in content.lower() else 'normal',
            'status': 'pending',
    ...
    def process_inbox(self, limit: int = 50) -> Dict:
        """Process recent emails and generate responses/tasks."""
        ...
        try:
            ...
            for email in emails:
                ...
                if classification['requires_reply']:
                    draft = self.generate_draft_reply(email_data, classification)
                    email_result['draft'] = draft
                    email_result['action_taken'] = 'draft_created'
                    results['drafts_created'] += 1
                
                elif classification['create_task']:
                    task = self.create_task_from_email(email_data)
                    email_result['task'] = task
                    email_result['action_taken'] = 'task_created'
                    results['tasks_created'] += 1
                
                elif classification['delegate_to']:
                    email_result['delegate_to'] = classification['delegate_to']
                    email_result['action_taken'] = 'delegated'
                    results['delegations'] += 1
                
                else:
                    email_result['action_taken'] = 'archived'
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Process emails and generate responses')
    ...
    print(f"🚀 Processing last {args.limit} emails...")
    
    results = processor.process_inbox(limit=args.limit)
    ...
    if args.output == 'json':
        ...
        print(f"├─ Delegations: {results['delegations']}")
        print(f"└─ Errors: {results['errors']}")
        
        print(f"\n📧 Email Actions:")
        for email in results['emails'][:10]:  # Show first 10
            print(f"\n{'='*60}")
            print(f"Subject: {email['subject']}")
            print(f"From: {email['sender']}")
            print(f"Category: {email['classification']['category']}")
            print(f"Confidence: {email['classification']['confidence']:.0%}")
            print(f"Action: {email['action_taken']}")
            ...
            if 'task' in email:
                print(f"\n📋 Created Task:")
                print(f"  Title: {email['task']['title']}")
                print(f"  Priority: {email['task']['priority']}")
